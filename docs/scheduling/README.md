...
- [学习策略与 MILP](learning-and-milp.md)：RL 的模型基础以及 `milp` 的建模决策。
...
代码入口：`src/schedule/api.py` 提供启发式与 RL 搜索入口，
`src/schedule/milp.py` 提供 MILP；`src/timing/solve.py` 只负责固定顺序定时。

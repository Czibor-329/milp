...
- **管理员（admin）**：拥有全部设备与算法权限，可进入「用户管理」分配权限；
- **普通用户（user）**：只能看到并使用管理员分配给自己的设备（含测试集）
  与算法，未分配的设备不可见、未分配的算法不可运行。权限修改即时生效。

### 网页端管理（推荐）

管理员登录后，在主页面左侧边栏点击「**用户管理**」，即可在页面里：
新建用户、设置角色、勾选允许的算法与设备、重置密码、删除账号。

> 普通用户登录后看不到「用户管理」入口。

### 命令行管理

也可以在项目目录下执行命令（新建账号默认是管理员角色，密码至少 8 位）：
...
建议为每个调度员建**普通用户**账号并分配权限，方便日后追溯与隔离；
改掉默认 admin 的密码，避免共享账号。

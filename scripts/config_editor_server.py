...
页面只负责编辑设备引用、通用 Recipe/Clean/Route 和各轮新增 Job；本服务把请求展开成
标准调度接口数据，依次运行首次排程与实时重算。设备及其测试集持久化在
results/config_editor_workspaces.json，最近的甘特图和复现日志只保存在内存中。

用法：
    python scripts/config_editor_server.py
    python scripts/config_editor_server.py --port 8765 --open
"""
...
WORKSPACE_STORE_PATH = ROOT / "results" / "config_editor_workspaces.json"
...
def _write_workspace_catalog_unlocked(path: Path, catalog: Mapping[str, Any]) -> None:
    ...
    temporary_path.write_text(
        json.dumps(catalog, ensure_ascii=False, allow_nan=False, indent=2),
        encoding="utf-8",
    )
...
def _normalize_test_case(raw_test: Mapping[str, Any], test_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "cleans": deepcopy(list(raw_test.get("cleans") or [])),
        "routes": deepcopy(list(raw_test.get("routes") or [])),
        "rounds": rounds,
...
def save_reproduction_log(entries: Sequence[Mapping[str, Any]]) -> str:
    """把 input_data 格式日志放入有界内存缓存并返回日志标识。"""
    log_id = uuid.uuid4().hex
    with _REPRODUCTION_LOGS_LOCK:
        _REPRODUCTION_LOGS[log_id] = deepcopy(list(entries))

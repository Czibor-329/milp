...
边界展示使能、物理拦截和死锁规则拦截动作。Pick 写到 Robot 手槽，例如
`Pick(1) LP1#1 → ATR#1`；相同库存站拦截会合并。原因在卡片悬浮时显示。
算法未实现该函数时动作卡片保持空白；回放页面不运行或展示推荐模型。
工具栏的“导出诊断包”会按当前时间轴位置下载 JSON，包含当前代 UpdateParams、
MoveStates、模块与手槽快照、Route 步骤、最近动作及全部候选动作与拦截原因。
